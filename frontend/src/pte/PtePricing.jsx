...
export const PtePricing = () => {
  ...
  return (
    <section id="pricing" className="relative bg-white rounded-4xl px-4 py-10 md:px-8 overflow-hidden font-sans">
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-emerald-100/10 rounded-full filter blur-3xl -z-10" />
      ...
      <div className="mx-auto max-w-6xl relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 md:mb-20">
          <div className="flex flex-col items-center gap-2 mb-4">
            <motion.span 
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-[#E30613] bg-red-50 border border-red-100 rounded-full"
            >
              <FiZap className="w-3.5 h-3.5 fill-current animate-pulse text-[#E30613]" />
              PTE Pricing & Plans
            </motion.span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl mx-auto">
          {ptePlans.map((plan, idx) => {
            const isPopular = idx === 1; // Second card is the featured package
            return (
              <motion.div
                key={plan._id || idx}
                  ...
                  <ul className="space-y-4 pt-4 border-t border-dashed border-current/20">
                    {plan.features?.map((feature, fIdx) => (
                      <li key={fIdx} className="flex items-start gap-3">
                        <FiCheck className={`w-5 h-5 shrink-0 ${isPopular ? "text-amber-300" : "text-emerald-500"}`} />
                        <span className="text-sm font-medium">{feature}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                <button className={`w-full mt-8 py-4 font-bold rounded-2xl flex items-center justify-center gap-2 transition-all cursor-pointer ${
                  isPopular 
                    ? "bg-white text-emerald-700 hover:bg-emerald-50" 
                    : "bg-[#E30613] text-white hover:opacity-90 shadow-md shadow-red-100"
                }`}>
